...
class AutoTrader:
    ...
    def transaction_through_bridge(self, pair: Pair, all_tickers):
        ...
        result = self.manager.buy_alt(
            pair.to_coin, self.config.BRIDGE, all_tickers)
        if result is not None:
            self.update_trade_threshold(
                pair.to_coin, float(result["price"]), all_tickers)
            return result
    ...
    def update_trade_threshold(self, coin: Coin, coin_price: float, all_tickers):
        ...
        if coin_price is None:
            self.logger.info("Skipping update... current coin {} not found".format(
        ...
        with self.db.db_session() as session:
            for pair in session.query(Pair).filter(Pair.to_coin == coin):
                from_coin_price = get_market_ticker_price_from_list(
                    all_tickers, pair.from_coin + self.config.BRIDGE)
        ...
        with self.db.db_session() as session:
            for pair in session.query(Pair).filter(Pair.ratio.is_(None)).all():
                if not pair.from_coin.enabled or not pair.to_coin.enabled:
                    continue
                self.logger.info(
                    f"Initializing {pair.from_coin} vs {pair.to_coin}", False)
                ...
                to_coin_price = get_market_ticker_price_from_list(
                    all_tickers, pair.to_coin + self.config.BRIDGE)
    ...
    def _get_ratios(self, coin: Coin, coin_price, all_tickers):
        ...
        for pair in self.db.get_pairs_from(coin):
            ...
            self.db.log_scout(pair, pair.ratio, coin_price,
                              optional_coin_price)
            ...
            ratio_dict[pair] = (
                coin_opt_coin_ratio - transaction_fee *
                self.config.SCOUT_MULTIPLIER * coin_opt_coin_ratio
            ) - pair.ratio
        return ratio_dict
    ...
    def _jump_to_best_coin(self, coin: Coin, coin_price: float, all_tickers):
        ...
        if ratio_dict:
            ...
            self.logger.info(
                f"Will be jumping from {coin} to {best_pair.to_coin_id}")
    ...
    def update_values(self):
        ...
        with self.db.db_session() as session:
            ...
            for coin in coins:
                ...
                cv = CoinValue(coin, balance, usd_value,
